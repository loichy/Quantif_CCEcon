...
for(i in 1:15){
  print(i)
  Corpus.CleanedNames.2 <- Corpus.CleanedNames.2 %>% # First create variable gender (takes value: NA, unknown, male and female)
    mutate(!!paste0("gender_", i) := ifelse(test = is.na(.[[paste0("f_name_", i)]]), # 1. if first name is missing
                                            yes = NA, 
                                            no = ifelse(
                                              test = .[[paste0("f_name_", i)]] %in% common_chinese_names, # 2. if first name is in the list of common chinese first names
                                              yes = "unknown", 
                                              no = ifelse(
                                                test = .[[paste0("fullname_", i)]] %in% top10_female_economists, #3. If full name is in the list of top 10% of female economists
                                                yes = "female", 
                                                no = ifelse(
                                                  test = !(.[[paste0("f_name_", i)]] %in% names_proba_df$name), #4. If first name is absent from the list of first name with measured gender probability
                                                  yes = NA,
                                                  no = case_when(
    ...
    mutate(!!paste0("proportion_male_", i) := ifelse(test = is.na(.[[paste0("f_name_", i)]]), # Create the variable that gives the probability that author is male, only if gender is assigned
                                                     yes = NA,
                                                     no = ifelse(
           ...
           !!paste0("proportion_female_", i) := ifelse(test = is.na(.[[paste0("f_name_", i)]]), # Create the variable that gives the probability that author is female, only if gender is assigned
                                                     yes = NA,
                                                     no = ifelse(
                                                       test = .[[paste0("f_name_", i)]] %in% common_chinese_names,
                                                       yes = NA,
...
122788 different authors for 60 000 articles.
...
NotGendered_authors #30 191 auteurs qui n'ont pas été assignés sur 111 116 auteurs différents (27% d'auteurs non assignés)
```
...
soit pivot_longer avec end_ith soit à partir de authors_df avec geom_bar
```{r} 
#authors_df %>%
  #summarize(n = n(), 
...
```{r}
stats_by_cat_articles <- Corpus.CleanedNames.2 %>%
  group_by(PY,category_gender) %>% summarize(nb_articles=n())

stats_by_cat_articles 
```

```{r}
x = PY, color=category_gender
```
...
Articles qui portent sur le CC et qui ne sont pas dans le top 30 : medianne de la proportion de femmes un peu plus élevée
