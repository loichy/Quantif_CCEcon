...
url <- "https://ideas.repec.org/top/top.person.all.html"  # Replace with your actual URL
...
top10_economists <- c(names_1[!is.na(names_1)], names_2) %>% 
  unique()

# Get male economists by substracting from this list the full names of the top 10% of female economists
load(here(dir$prep.data, "FemaleEconomists_Names.Rdata"))
top10_male_economists <- top10_economists[!(top10_economists %in% top10_female_economists)]


save(top10_male_economists, file = here(dir$prep.data, "MaleEconomists_Names.Rdata"))
