...
-   U1 et U2 peuvent être des variables intéressantes mais je ne comprends pas vraiment à quoi elles font référence
...
sample_data <- Corpus.CleanedNames.2 %>% slice(1:6000)
```

```{r}
sample_data2 <- sample_data %>%
  mutate(PY = as.factor(PY),    # Effet fixe année
         TI = as.factor(TI))  # Effet fixe journal
```

```{r}
levels_count <- sample_data %>%
  summarise(TI_levels = n_distinct(TI),
            PY_levels = n_distinct(PY))

print(levels_count)
```

```{r}
model2 <- lm(TC ~ TI + PY + proportion_gender_female_id + PG + nb_authors, data = sample_data)
```

```{r}
# Installer et charger la bibliothèque broom
#install.packages("broom")
#library(broom)

# Convertir les résultats du modèle en dataframe
...
proportion_gender_female_id a un coefficient positif et significatif et la variable nb_authors aussi.

### Dummy Variable Regression

```{r}
model = list()
model$dummy <- lm(TC ~ + proportion_gender_female_id + PG + nb_authors + factor(TI)+ factor(PY), data = sample_data)
```
...
sample_data2 <- sample_data %>%
  group_by(TI) %>%
  mutate(mean_TC = mean(TC),
         mean_prop = mean(proportion_gender_female_id),
         mean_PG = mean(PG),
         mean_nb_authors = mean(nb_authors)) %>%
  mutate(demeaned_TC = TC - mean_TC,
         demeaned_prop = proportion_gender_female_id - mean_prop,
         demeaned_PG = PG - mean_PG,
         demeaned_nb_authors = nb_authors - mean_nb_authors)
```
...
```{r}
extract_countries <- function(affiliation_text) {
    found_countries <- character(0)
    for (country in country_list) {
        if (grepl(country, affiliation_text, ignore.case = TRUE)) {
...
```{r}
affiliations <- c(
    "[Rozmahel, Petr] Mendel Univ, Fac Business & Econ, Res Ctr, Brno, Czech Republic",
    "Szent Istvan Univ, Inst European Studies, Budapest, Hungary",
    "[Kundera, Jaroslaw] Univ Wroclaw, Inst Econ, Wroclaw, Poland",
    "Susquehanna Univ, Selinsgrove, PA 17870 USA",
    "Austrian Natl Bank, Vienna, Austria",
    "[Sojka, Milan; Sevela, Marcel] Mendel Univ, Brno, Czech Republic",
    "[Ogawa, Eiji] Hitotsubashi Univ, Grad Sch Commerce & Management, Tokyo, Japan; [Kumamoto, Masao] Tokyo Keizai Univ, Fac Econ, Tokyo, Japan",
    "[Smrckova, Gabriela] Univ Econ Prague, Prague, Czech Republic",
    "[Starzynska, Waclawa; Zrobek, Janusz] Jan Kochanowski Univ Humanities & Sci, Piotrkow Trybunalski, Poland"
)

country_per_affiliation <- sapply(affiliations, extract_countries)

# Résultat : une liste où chaque élément correspond aux pays trouvés dans chaque ligne de C1
country_per_affiliation

```

```{r}
sample_data <- sample_data %>%
    mutate(Country = sapply(C1, extract_countries))
```

```{r}
head(sample_data$Country, 30)
```


Que faire en suite?
1.
Décrire les données: part des articles provenant de chaque pays du monde ou provenant de certaines régions du monde (US, vs EU, vs Reste du monde), évolution de ces parts par année.
